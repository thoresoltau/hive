...
class ScrumMasterAgent(BaseAgent):
    ...
    async def _select_next_ticket(self) -> AgentResponse:
        ...
        if ready_ticket:
            return AgentResponse(
                ...
                message=f"Ticket {ready_ticket.id} is ready for implementation.",
            )
        ...
        if backlog_ticket:
            return AgentResponse(
                ...
                message=f"Ticket {backlog_ticket.id} needs to be refined.",
            )
        ...
        return AgentResponse(
            ...
            message="No tickets available in backlog.",
        )
    ...
    async def _start_refinement(self, ticket_id: Optional[str]) -> AgentResponse:
        ...
        ticket.add_comment(
            self.name,
            "Refinement process started. Handover to Product Owner."
        )
        await self.backlog.save_ticket(ticket)

        response = await self.handoff_to(
            target_agent="product_owner",
            task_description="Please refine this ticket with acceptance criteria and user story.",
            ticket_id=ticket_id,
        )
        ...
        return AgentResponse(
            ...
            next_agent="product_owner",
            message=f"Refinement for {ticket_id} started.",
        )
    ...
    async def _check_blockers(self) -> AgentResponse:
        ...
        analysis = await self._call_llm(
            user_message=f"""
            Analyze the following blockers and propose solutions:
        ...
        return AgentResponse(
            ...
            message=f"{len(blocked_tickets)} blocked tickets found.",
        )
    ...
    async def _run_sprint_planning(self) -> AgentResponse:
        ...
        if not available_tickets:
            return AgentResponse(
                ...
                message="No refined tickets available for sprint planning.",
            )
        ...
        planning_result = await self._call_llm_json(
            user_message=f"""
            Sprint Planning: Prioritize the following tickets for the next sprint.
            ...
            2. Dependencies (blocked_by must be empty or already planned)
            3. Complexity and Story Points
        ...
        return AgentResponse(
            ...
            message=f"Sprint Planning completed. {len(planning_result.get('sprint_tickets', []))} tickets planned.",
        )
    ...
    async def _orchestrate_workflow(self) -> AgentResponse:
        ...
        if review_tickets:
            ...
            return AgentResponse(
                ...
                next_agent="architect",
                message=f"Ticket {ticket.id} is waiting for code review.",
            )
        ...
        if in_progress_tickets:
            ...
            if assigned_to:
                return AgentResponse(
                    ...
                    message=f"Ticket {ticket.id} delegated to {assigned_to} for further work.",
                )
            else:
    ...
    async def _check_and_handle_loop(self, ticket) -> bool:
        ...
        if cycle_count > self.MAX_CYCLES_PER_TICKET:
            ...
            self.log.warning(
                f"🔄 Loop detected: Ticket {ticket_id} has {cycle_count} cycles. "
                f"Ticket is blocked."
            )
            ...
            ticket.add_comment(
                self.name,
                f"⚠️ Ticket blocked due to loop ({cycle_count} cycles). "
                f"Manual review required. Last status before block: {ticket.status.value}"
            )
            await self.backlog.save_ticket(ticket)
    ...
    async def _assign_developer(self, ticket) -> AgentResponse:
        ...
        return AgentResponse(
            ...
            result={"ticket_id": ticket.id, "assigned_to": assigned_to},
            next_agent=assigned_to,
            message=f"Ticket {ticket.id} assigned to {assigned_to}.",
        )
